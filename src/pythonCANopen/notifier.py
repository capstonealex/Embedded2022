...
class Notifier(object):
    ...
    def add_bus(self, bus):
        ...
        if self._loop is not None and hasattr(bus, 'fileno') and bus.fileno() >= 0:
            ...
            notifierProcess = bus.fileno()
            logger.info("loop")
            logger.info(self._loop)
            self._loop.add_reader(notifierProcess, self._on_message_available, bus)
        else:
            q = Queue()
            notifierProcess = Process(target=self._rx_thread, args=(bus, self.queue),
                name='can.notifier for bus "{}"'.format(bus.channel_info))
            notifierProcess.daemon = True
            self._readers.append(notifierProcess)
            readerCallbackThread1 = threading.Thread(target=self._rx2_thread, args=(self.queue,),
                name='can.call back handler1')
            readerCallbackThread1.daemon = True
            readerCallbackThread2 = threading.Thread(target=self._rx2_thread, args=(self.queue,),
                name='can.call back handler2')
            readerCallbackThread2.daemon = True
            self._threads = [readerCallbackThread1, readerCallbackThread2]
            notifierProcess.start()
            readerCallbackThread2.start()
            readerCallbackThread1.start()
            

    def stop(self, timeout=5): #od not called
