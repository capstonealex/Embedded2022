...
class Converter():
    ...
    def position(self, rawmsg):
        position = int.from_bytes(rawmsg[0:4], byteorder='little', signed=True) 
        #print(position)
        return position

    def velocity(self, rawmsg):
        velocity = int.from_bytes(rawmsg[4:8], byteorder='little', signed=True)
       # print(velocity)
        return 0 #velocity
    
    def torque(self, rawmsg):
        torque = int.from_bytes(rawmsg, byteorder='little', signed=True)
        #print(torque)
        return 0 #torque
    ...
    def Left_crutch_data_2(self, data_1, rawmsg):
        ...
        #     data_1[i+7] = rawmsg[i]
        # for i in range(0,6):
        #     self.Left_crutch_data[i] = int.from_bytes(data_1[0+2*i:2+2*i], byteorder='big', signed=True)
    ...
    def Right_crutch_data_2(self, data_1, rawmsg):
        ...
        #     data_1[i+7] = rawmsg[i]
        # for i in range(0,6):
        #     self.Right_crutch_data[i] = int.from_bytes(data_1[0+2*i:2+2*i], byteorder='big', signed=True)
